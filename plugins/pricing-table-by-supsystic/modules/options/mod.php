		...
		if(empty($this->_options)) {
			$this->_options = dispatcherPts::applyFilters('optionsDefine', array(
				'general' => array(
					...
					'opts' => array(
						'send_stats' => array('label' => __('Send usage statistics', PTS_LANG_CODE), 'desc' => __('Send information about what plugin options you prefer to use, this will help us make our solution better for You.', PTS_LANG_CODE), 'def' => '0', 'html' => 'checkboxHiddenVal'),
						...
						//'access_roles' => array('label' => __('User role can use plugin', PTS_LANG_CODE), 'desc' => __('User with next roles will have access to whole plugin from admin area.', PTS_LANG_CODE), 'def' => 'administrator', 'html' => 'selectlist', 'options' => array($this, 'getAvailableUserRolesSelect'), 'pro' => ''),
					),
				),
			));
